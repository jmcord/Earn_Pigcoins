...
import smart_contract_abi from './contracts/SmartContract.json';
...
// Importa las funciones necesarias para obtener el saldo de tokens en staking y recompensas acumuladas
import { getStakingBalance, getRewardsBalance } from './SmartContractFunctions';
...
class App extends Component {
  state = {
    ...
    userStakingBalance: 0, // Nuevo estado para almacenar el saldo de tokens en staking del usuario
    userRewardsBalance: 0 // Nuevo estado para almacenar el saldo de recompensas acumuladas del usuario
  };
  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      ...
      // Obtener y establecer el saldo de tokens en staking del usuario
      const userStakingBalance = await getStakingBalance(this.state.account, contract);
      this.setState({ userStakingBalance });

      // Obtener y establecer el saldo de recompensas acumuladas del usuario
      const userRewardsBalance = await getRewardsBalance(this.state.account, contract);
      this.setState({ userRewardsBalance });
  ...
  calculateReward = async (account) => {
    try {
      ...
      const reward = (await contract.methods.stakingBalance(account).call() * APY * timeElapsed) / (365 * 24 * 60 * 60 * 100); //
      return reward;
    } catch (err) {
      console.error(err);
      Swal.fire({
          ...
          <div className="row">
            <main role="main" className="col-lg-12 d-flex text-center">
              <div className="content mr-auto ml-auto">
                ...
                <h3>Saldo de Tokens en Staking del Usuario</h3>
                <p>{this.state.userStakingBalance} tokens</p>
                <h3>Saldo de Recompensas Acumuladas del Usuario</h3>
                <p>{this.state.userRewardsBalance} tokens</p>
              </div>
            </main>
          </div>
        </div>
